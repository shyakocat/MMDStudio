...
const {infoMmd, createMmd} = require('../Model/Mmd/MmdSh');
...
function loadDB() {
    ...
    let res;
    try {
        res = JSON.parse(fs.readFileSync(dbfile).toString());
    } catch (e) {
        console.log(e);
    }
...
DBItem.prototype.create = function (url) {
    ...
    function dealModel(url, suf, info, create) {
        ...
        if (t) {
            _this.name = t.name;
            _this.comment = t.comment;
        }
        return true;
    }

    if (dealModel(url, '.pmx', infoMmd, createMmd)) return;
...
DBItem.prototype.preview = function () {
    ...
    switch (this.suf()) {
        case "pmx":
            global_create = createMmd;
            break;
        case "pmd":
            global_create = createPmd;
            break;
        case "obj":
            global_create = createObj;
            break;
        case "x":
            global_create = createX;
            break;
        default:
            global_create = null;
    }
    ...
    return "<div align='center'>\
            ...
            <button onclick='\
                ...
                saveCanvasSync(global_canvas,switch_electronbuild?\"resources/app/\":\"\"+global_item.thumbnail);\
